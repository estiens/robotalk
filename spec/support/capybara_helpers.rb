module CapybaraHelpers
  ...
  def find_test_element(test_id, timeout: DEFAULT_TIMEOUT, **options)
    find("[data-test='#{test_id}']", wait: timeout, **options)
  end

  # Check if element with data-test attribute exists
  def has_test_element?(test_id, timeout: DEFAULT_TIMEOUT, **options)
    page.has_selector?("[data-test='#{test_id}']", wait: timeout, **options)
  end

  # Check if element with data-test attribute does not exist
  def has_no_test_element?(test_id, timeout: DEFAULT_TIMEOUT, **options)
    page.has_no_selector?("[data-test='#{test_id}']", wait: timeout, **options)
  end

  # Click element by data-test attribute
  def click_test_element(test_id, timeout: DEFAULT_TIMEOUT, **options)
    find_test_element(test_id, timeout: timeout, **options).click
  end
  ...
  def wait_for_test_element(test_id, timeout: DEFAULT_TIMEOUT, **options)
    expect(page).to have_selector("[data-test='#{test_id}']", wait: timeout, **options)
  end

  # Wait for element to disappear with data-test attribute
  def wait_for_test_element_removal(test_id, timeout: DEFAULT_TIMEOUT, **options)
    expect(page).to have_no_selector("[data-test='#{test_id}']", wait: timeout, **options)
  end
  ...
  def within_conversation_card(conversation_id, &block)
    within("[data-test='conversation-card-#{conversation_id}']", &block)
  end
  ...
  def expect_flash_message(message, type: :notice, timeout: DEFAULT_TIMEOUT)
    expect(page).to have_selector(".flash-#{type}", text: message, wait: timeout)
  end

  # Database assertion helpers
  def expect_record_count_change(model_class, change_by:, &block)
    expect { block.call }.to change { model_class.count }.by(change_by)
  end
