...
RSpec.describe "Conversation deletion with JavaScript confirmation", type: :feature, js: true do
  around(:each) { |example| pending("PENDING: JS confirmation dialogs not working with Cuprite"); example.run }
  ...
  describe "with JavaScript enabled" do
    it "shows confirmation dialog and deletes on accept" do
      ...
      # Verify deletion
      expect(page).not_to have_content("Test Topic for JS")
  ...
  describe "hover behavior for delete button" do
    it "reveals delete button on hover", js: true do
  ...
  describe "multiple conversation selection" do
    it "can delete multiple conversations in sequence" do
      conversation1 = create_test_conversation(user: user, topic: "First Topic")
      conversation2 = create_test_conversation(user: user, topic: "Second Topic")
      conversation3 = create_test_conversation(user: user, topic: "Third Topic")
